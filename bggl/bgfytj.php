			...
			<div>
				...
				<div layoutH="146" style="float:left; display:block; overflow:auto; width:240px; border:solid 1px #CCC; line-height:21px; background:#fff">
				    ...
				    for($a=0;$a<count($resultarray);$a++){ ?>
							<ul><li>
						<a href="javascript:;"  rel="bgfytjbox">
				...
				<div id="bgfytjbox" class="unitBox" style="margin-left:246px;">
	...
	<table class="table" width="100%" layoutH="138" style="word-break:break-all; word-wrap:break-all;">
		...
		<tbody>
			 ...
			 require_once $_SESSION["ROOT"].'/db/db.php';
			     $result=mysqli_query($con,"select * from t_subjectset" );
			     $resultarray=mysqli_fetch_all($result,MYSQLI_ASSOC);
			     //分页显示
			     $resultnum=count($resultarray);
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_subjectset order by id DESC limit ".$sr.",".$numPerPage  );
...
<tr  >
			<td ><?php echo ($a+1);?>
			</td><td ><?php echo $resultnowarray[$a]['subjectName'];?>
            </td><td ><?php 
            $allMoneysql=mysqli_query($con, "select borrowMoney,loanMoney from t_officeaccount where subjectType_id=".$resultnowarray[$a]['id']);
            $allMoney=mysqli_fetch_all($allMoneysql,MYSQLI_ASSOC);
            for($all=0;$all<count($allMoney);$all++){
                $jie+=$allMoney[$all]["borrowMoney"];
                $dai+=$allMoney[$all]["loanMoney"];
            }
            echo $jie;
            ?>
			</td><td  ><?php echo $dai;?>
			</td><td  ><?php echo $jie-$dai;?>
			</td></tr>
		<?php 	
		$jieh+=$jie;
		$daih+=$dai;
    ...
    ?>
			<tr class="tfoot">
				...
				<th align="center"><?php echo $jieh-$daih;?></th>
	...
	.tfoot:hover{
		...
		<div class="pages">
			<span>显示</span>
			<select class="combox" name="numPerPage" onchange="navTabPageBreak({numPerPage:this.value},'bgfytjbox')">
		...
		<div class="pagination" targetType="bgfytjbox" totalCount="<?php echo count($resultarray); ?>" numPerPage="<?php echo $numPerPage;?>" pageNumShown="10" currentPage="<?php echo $pageNum;?>"></div>

...
");
    if($borrowMoney!=null){
        mysqli_query($con, "update t_account set money=money+".$borrowMoney." where id= ".$acount);
        mysqli_query($con, "insert into t_moneychange(accountid,km,min,dotime,douser,changetype)values('".$acount."',
'办公费用收入','".$borrowMoney."','".$creattime."','".$operator."','in')");
    }elseif ($loanMoney!=null){
        mysqli_query($con, "update t_account set money=money-".$loanMoney." where id= ".$acount);
        mysqli_query($con, "insert into t_moneychange(accountid,km,mout,dotime,douser,changetype)values('".$acount."',
'办公费用支出','".$loanMoney."','".$creattime."','".$operator."','out')");
    }
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="bggl/bgfylr.php" method="post" >
	<div class="searchBar">
		...
		<table class="searchContent">
			<tr>
				...
				<td class="dateRange">
					会计期间选择:<input type="text" name="handleDate" value="<?php 
					echo isset($_POST["handleDate"])?$_POST["handleDate"]:date("Y-m",time());?>" class="date" datefmt="yyyy-MM" size="30" style="width: 152px;height:21px;" readonly /><button type="submit">检索</button>
				</td>
			</tr>
	...
	<table class="table" width="100%" layoutH="138" style="word-break:break-all; word-wrap:break-all;">
		...
		<tbody>
			 ...
			 if(@$_POST["search"]==null){
			     ...
			     $result=mysqli_query($con,"select * from t_officeaccount where handleDate=  '".$firstday."'" );
			     $resultarray=mysqli_fetch_all($result,MYSQLI_ASSOC);
			     //分页显示
			     $resultnum=count($resultarray);
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_officeaccount where handleDate=  '".$firstday."' order by id DESC limit ".$sr.",".$numPerPage  );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }else {
			     $sql="";
			     if($_POST["handleDate"]!=""){
			         $startdate=$_POST["handleDate"];
			         $sql.=" and handleDate=  '".$startdate."'";
			     }
			     $result=mysqli_query($con,"select * from t_officeaccount where 1=1 ".$sql );
			     ...
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_officeaccount where 1=1 ".$sql." limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }

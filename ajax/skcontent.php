...
// 			 $yshdd=explode(",", $allddid);//所有已经审核过的订单
			 if(@$_GET["search"]==null){
			     $result=mysqli_query($con,"select * from t_groupmanage 
			     		where groupName='".$travel."' and  startDate between '".$firstday."' and '".$today."'" );
			     ...
			     $resultnow=mysqli_query($con,"select * from t_groupmanage  where groupName='".$travel."' 
			     ...
			     $result=mysqli_query($con,"select * from t_groupmanage  where groupName='".$travel."'
 ".$sql." order by id" );
...
for($w=0;$w<count($zm);$w++){
 ...
 where groupName='".$travel."' ".$sql." order by startDate desc limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }

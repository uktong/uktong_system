...
function getjdian(id){
 ...
 <form  onsubmit="return navTabSearch(this);" class="pageForm" action="xyzx/cbcgxy.php?action=edit&id=<?php echo $id;?>" method="post"  enctype="multipart/form-data">
...
<!--                          <input name="hotelName" type="text" id="dutycode" ltype="text" ligerui="{width:160}" class="required "  /> -->
                         <input type='hidden' name='jdian112.id' value=' <?php echo $xy['hotelName'];?>'/>
                         <input type='text' oninput="getjdian('112')" readonly class="getjdian112 requerd" 
                         name='jdian112.jdian112' value='  <?php 
                        ...
                        ?>'  style='width: 80%;' suggestFields='jdian112'  lookupGroup='jdian112' />
                    </td>
                ...
                ?>
                 <td align="right" class="editcellmessage" style="width:15%;"><?php echo $fx[$a]["basetype"];?>：</td>
                   ...
                   $jgsql=mysqli_query($con, "select price from t_roomprice where roomType='".$fx[$a]["id"]."' and hotelSchemeId=".$id);

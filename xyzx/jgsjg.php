...
if(isset($_GET["action"])){
    if($_GET["action"]=="charu"){
        ...
        if($travelName!=""){
            ...
            mysqli_query($con, "insert into t_protocol(dealName,jgs,starttime,endtime,remark,flag,issuer)
values('".$dealName."','".$travelName."','".$starttime."','".$endtime."','".$remark."','".$flag."','".$user."')
...
<form id="pagerForm" method="post" action="xyzx/jgsjg.php">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="xyzx/addjgsxy.php" target="navTab" rel="addxy"  title="添加协议" width="576" height="544"> <span>添加</span></a></li>
	...
	<table class="table" width="100%" layoutH="138" style="word-break:break-all; word-wrap:break-all;">
		...
		<tbody>
			 ...
			 if(@$_POST["search"]==null){
			     $result=mysqli_query($con,"select * from t_protocol where jgs is not null" );
			     $resultarray=mysqli_fetch_all($result,MYSQLI_ASSOC);
			     //分页显示
			     $resultnum=count($resultarray);
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_protocol where jgs is not null order by id DESC limit ".$sr.",".$numPerPage  );
			     ...
			     $result=mysqli_query($con,"select * from t_protocol  where jgs is not null ".$sql." order by id" );
			     ...
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_protocol where jgs is not null ".$sql." order by id DESC limit ".$sr.",".$numPerPage  );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }
...
for($cost=0;$cost<count($price);$cost++){
    ...
    echo $roomtype["basetype"]."：".$price[$cost]["price"]."元<br>";
			...
			<a href='xyzx/showjgs.php?id=<?php echo $resultnowarray[$a]['id'];?>' class='show' id='' mask="true" rel="showjgsxy" target='dialog' title='查看协议<?php echo $resultnowarray[$a]['id'];?>' style='color:blue;'>查看</a>
		<a href='xyzx/editjgsxy.php?id=<?php echo $resultnowarray[$a]['id'];?>' class='show' id='' mask="true" rel="editjgsxy" width="610" target='dialog' title='编辑协议<?php echo $resultnowarray[$a]['id'];?>' style='color:blue;'>编辑</a>
						<a href='xyzx/jgsjg.php?id=<?php echo $resultnowarray[$a]['id'];?>&action=delete' class='show' id='' target="navTab" rel="lxsjg" title='编辑协议<?php echo $resultnowarray[$a]['id'];?>' style='color:blue;'>删除</a>
			</td></tr>
		<?php 	}

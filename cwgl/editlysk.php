...
<form method="post" action="db/addlysk.php?action=edit&id=<?php echo $id; ?>" onsubmit="return validateCallback(this, dialogAjaxDone) " class="pageForm required-validate" >
<div class="pageHeader">
<h2 class="contentTitle" style="text-align: center;">修改旅游收款</h2>
	...
	<div class="searchBar">
		...
		<table class="searchContent" id="fftable"  cellspacing="0">
			<tr>
				<td >
					客人类型:
				<input type="hidden" name="zts.id" id="travelsk" value="<?php echo @$getmsg["sk"];?>"/>
				<input type="text" class="getzts" name="zts.zts" value="散客" readonly style="width:100px; " suggestFields="zts"  lookupGroup="zts" />
...
select a.*,sum(ifnull(b.sumPrice,0)) as allin  from t_groupmanage as a right join t_reserveplan as b on a.teamNumber=b.groupNumber where a.hotelManage='代订旅游' and a.groupName='".$getmsg["sk"]."' and b.groupNumber=a.teamNumber
...
and  b.startDate between '".$firstday."' and '".$today."' group by a.teamNumber
 order by a.startDate desc  limit ".$sr.",".$numPerPage  );
			     ...
			     $result=mysqli_query($con,"select a.*,b.sumPrice as allin  from t_groupmanage as a right join t_reserveplan as b on a.teamNumber=b.groupNumber where  a.hotelManage='代订旅游' and a.groupName='".$getmsg["sk"]."' and b.groupNumber=a.teamNumber
...
for($w=0;$w<count($zm);$w++){
	$yifu+=$zm[$w]["amount"];
			     ...
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select a.*,b.sumPrice as allin  from t_groupmanage as a right join t_reserveplan as b on a.teamNumber=b.groupNumber where a.hotelManage='代订旅游' and a.groupName='".$getmsg["sk"]."' and b.groupNumber=a.teamNumber
...
<!-- 				<th align="center">销售</th> -->
		...
		<tbody id="datacontent" >
		 <?php 
			 ...
			 if(@$_POST["search"]==null){
			     $result=mysqli_query($con,"select a.*,sum(ifnull(b.sumPrice,0)) as allin  from t_groupmanage as a right join t_reserveplan as b on a.teamNumber=b.groupNumber where  a.hotelManage='代订旅游' and a.groupName='".$getmsg["jgs"]."' and b.groupNumber=a.teamNumber
and  b.startDate between '".$firstday."' and '".$today."' group by a.teamNumber" );
			     $resultarray=mysqli_fetch_all($result,MYSQLI_ASSOC);
			     //分页显示
...
and  b.startDate between '".$firstday."' and '".$today."' group by a.teamNumber
 order by a.startDate desc  limit ".$sr.",".$numPerPage  );
			     ...
			     $result=mysqli_query($con,"select a.*,sum(ifnull(b.sumPrice,0)) as allin  from t_groupmanage as a right join t_reserveplan as b on a.teamNumber=b.groupNumber where  a.hotelManage='代订旅游' and a.groupName='".$getmsg["sk"]."'and b.groupNumber=a.teamNumber
 ".$sql." order by a.id" );
...
for($w=0;$w<count($zm);$w++){
	$yifu+=$zm[$w]["amount"];
			     ...
			     $resultnow=mysqli_query($con,"select a.*,sum(ifnull(b.sumPrice,0)) as allin  from t_groupmanage as a right join t_reserveplan as b on a.teamNumber=b.groupNumber where  a.hotelManage='代订旅游' and a.groupName='".$getmsg["sk"]."' and b.groupNumber=a.teamNumber

...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="cwgl/groupout.php" method="post" >
	<div class="searchBar">
		...
		<table class="searchContent">
			<tr>
				...
				<td class="dateRange">
					按首晚入住日期:
					<input name="startDate" class="date readonly" readonly="readonly" type="text" value="<?php echo $firstday;?>">
					<span class="limit">-</span>
					<input name="endDate" class="date readonly" readonly="readonly" type="text" value=" <?php echo $lastday;?>">
				</td>
				<td >
					操作人:
	...
	<table class="table" width="100%" layoutH="138" style="word-break:break-all; word-wrap:break-all;">
		...
		<tbody>
			 ...
			 if(@$_POST["search"]==null){
			     $result=mysqli_query($con,"select * from t_groupmanage where startDate between '".$firstday."' and '".$lastday."'" );
			     ...
			     $resultnow=mysqli_query($con,"select * from t_groupmanage where startDate between '".$firstday."' and '".$lastday."' order by id DESC limit ".$sr.",".$numPerPage  );
			...
			for($a=0;$a<count($resultnowarray);$a++){
			    ...
			    $zmsql=mysqli_query($con, "select * from t_hoteldebt where groupNumber='".$resultnowarray[$a]['teamNumber']."'");
			    ...
			    for($q=0;$q<count($plan);$q++){
			        $yingfu+=$plan[$q]["hotelCommissionSum"];
			    }
			    $qianfu=$yingfu-$yifu;
...
<tr  >
			...
			<a href='cwgl/dogroupout.php?id=<?php echo $resultnowarray[$a]['id'];?>' class='show' id='' target='navtab' title='付款<?php echo $resultnowarray[$a]['id'];?>' style='color:blue;'>付款</a>
		<?php 	}

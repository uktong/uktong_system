...
if(isset($_POST["zts_id"])){
    ...
    mysqli_query($con, "insert into t_traveluser(realname,username,password,phone,tel,fax,travel,status,creattime)values(
...
'".$txtUserTel."','".$txtUserFax."','".$hotel."','".$chkUseChk."',now()
    ...
    if($_GET["action"]=="qxgl"){
        ...
        mysqli_query($con, "update t_traveluser set userlimit='".$allqx."' where id=".$_GET["id"]);
//         echo "update t_traveluser set userlimit='".$allqx."' where id=".$_GET["id"];
    }
    if($_GET["action"]=="delete"){
        mysqli_query($con, "delete from t_traveluser  where id=".$_GET["id"]);
    ...
    if($_GET["action"]=="edit"){
        ...
        $txtUserFax=$_POST["txtUserFax"];
        $txtUserMobile=$_POST["txtUserMobile"];
        mysqli_query($con, "update t_traveluser  set realname='".$txtUserName."',username='".$txtUserAccount."',password='".$txtUserPwd."',
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="zhzx/addtraveluser.php" target="dialog" mask="true" rel="qwewq" title="添加旅行社用户账号" width="670" height="220"><span>添加</span></a></li>
	...
	<table class="table" width="100%" layoutH="138" style="word-break:break-all; word-wrap:break-all;">
		...
		<tbody id="datacontent" >
			...
			if(@$_POST["search"]==null){
			    $result=mysqli_query($con,"select * from t_traveluser" );
			    $resultarray=mysqli_fetch_all($result,MYSQLI_ASSOC);
			    //分页显示
			    $resultnum=count($resultarray);
			    @$page=ceil($resultnum/$numPerPage);
			    @$sr=($pageNum-1)*$numPerPage;
			    $resultnow=mysqli_query($con,"select * from t_traveluser limit ".$sr.",".$numPerPage );
...
// 			    $sql.=$_POST["name"]!=""?" and updatePeople='".$_POST["name"]."'":"";
			    ...
			    @$page=ceil($resultnum/$numPerPage);
			    @$sr=($pageNum-1)*$numPerPage;
			    $resultnow=mysqli_query($con,"select * from t_traveluser where 1=1 ".$sql." limit ".$sr.",".$numPerPage );
			    $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			}

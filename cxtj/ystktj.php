...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" id="ystktjform" action="cxtj/ystktj.php" method="post" >
	<div class="searchBar">
	...
	<table class="table" width="100%" layoutH="158" style="word-break:break-all; word-wrap:break-all;">
		...
		<tbody>
			 ...
			 if(@$_POST["search"]==null){
			     $result=mysqli_query($con,"select distinct groupName from t_groupmanage where hotelManage='代订酒店' and groupName is not null and groupName!='' and  startDate between '".$firstday."' and '".$lastday."'" );
			     ...
			     $resultnow=mysqli_query($con,"select distinct groupName from t_groupmanage  where hotelManage='代订酒店' and groupName is not null and groupName!='' and  startDate between '".$firstday."' and '".$lastday."' order by id DESC limit ".$sr.",".$numPerPage  );
			     ...
			     $result=mysqli_query($con,"select distinct groupName from t_groupmanage where hotelManage='代订酒店' and groupName is not null and groupName!=''  ".$sql );
			     ...
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select distinct groupName from t_groupmanage where hotelManage='代订酒店' and groupName is not null and groupName!='' ".$sql." order by id DESC limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }

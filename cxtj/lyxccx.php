...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" id="jdyfcxform" action="cxtj/lyxccx.php" method="post" >
	<div class="searchBar">
...
<th align="center">欠付</th>
		...
		<tbody>
			 ...
			 if(@$_POST["search"]==null){
			     $result=mysqli_query($con,"select * from t_groupmanage right join t_reserveplan on t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订旅游' and t_reserveplan.startDate between '".$firstday."' and '".$lastday."'" );
...
t_groupmanage.teamNumber=t_reserveplan.groupNumber  where t_groupmanage.hotelManage='代订旅游' and t_reserveplan.startDate between '".$firstday."' and '".$lastday."' order by t_reserveplan.startDate desc  limit ".$sr.",".$numPerPage  );
...
t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订旅游'  ".$sql." order by t_groupmanage.id" );
 ...
 groupnumber='".$resultarray[$z]["teamNumber"]."' and reserveplan=".$resultarray[$z]["id"]);
			     ...
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_groupmanage right join t_reserveplan on
t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订旅游'  ".$sql." order by t_reserveplan.startDate desc limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }

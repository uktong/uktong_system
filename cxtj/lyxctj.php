...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="cxtj/lyxctj.php" id="jdyftjform" method="post" >
	<div class="searchBar">
...
<th align="center">数量</th>
		...
		<tbody>
			 ...
			 if(@$_POST["search"]==null){
			     $result=mysqli_query($con,"select distinct hotelName from t_reserveplan where type='ly' and startDate between '".$firstday."' and '".$lastday."'" );
			     ...
			     $resultnow=mysqli_query($con,"select distinct hotelName from t_reserveplan  where type='ly' and startDate between '".$firstday."' and '".$lastday."' order by id DESC limit ".$sr.",".$numPerPage  );
			    ...
			    // $sql.=$_POST["wl_id"]!=""?" and wl='".$_POST["wl_id"]."'":"";
			     ...
			     $result=mysqli_query($con,"select distinct hotelName from t_reserveplan where type='ly'  ".$sql );
			     ...
			     @$page=ceil($resultnum/$numPerPage);
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select distinct hotelName from t_reserveplan where type='ly'  ".$sql." order by id DESC limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }
			...
			?>
			<a href="javascript:print(<?php echo $jddianid;?>);"  style="color:blue;"><?php echo $jddian["travel_name"];?></a>
			</td>
<td  ><?php 

...
<form id="pagerForm" method="post" action="cxtj/jdyfcx.php">
<!-- 	<input type="hidden" name="status" value="${param.status}"> -->
<!-- 	<input type="hidden" name="keywords" value="${param.keywords}" /> -->
	<input type="hidden" name="pageNum" value="1" />
	<input type="hidden" name="numPerPage" value="<?php echo $numPerPage;?>" />
	<input name="startDate" class="date readonly" readonly="readonly" type="hidden" value="<?php
					echo  isset($_POST["startDate"])?$_POST["startDate"]:$firstday;?>">
	<input name="endDate" class="date readonly" readonly="readonly" type="hidden" value="<?php echo isset($_POST["endDate"])?$_POST["endDate"]:$lastday;?>">
	...
	<input name="groupnum"  type="hidden" size="30" value="<?php echo @$_POST["groupnum"];?>" />
	<input name="cusname"  type="hidden" size="30" value="<?php echo @$_POST["cusname"];?>" />
	<input type="hidden" name="zts.id" value="<?php echo @$_POST["zts_id"];?>"/>
	<input name="search"  type="hidden" size="30" value="yes"/>
</form>

<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" id="jdyfcxform" action="cxtj/jdyfcx.php" method="post" >
	<div class="searchBar">
...
t_groupmanage.teamNumber=t_reserveplan.groupNumber  where t_groupmanage.hotelManage='代订酒店' and t_reserveplan.startDate between '".$firstday."' and '".$lastday."' order by t_reserveplan.startDate desc  limit ".$sr.",".$numPerPage  );
...
t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订酒店'  ".$sql." order by t_groupmanage.id" );
 ...
 groupnumber='".$resultarray[$z]["teamNumber"]."' and reserveplan=".$resultarray[$z]["id"]);
			     ...
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_groupmanage right join t_reserveplan on
t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订酒店'  ".$sql." order by t_reserveplan.startDate desc limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }

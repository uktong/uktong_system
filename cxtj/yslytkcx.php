...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="cxtj/yslytkcx.php" id="searchform" method="post" >
	<div class="searchBar">
...
t_groupmanage.teamNumber=t_reserveplan.groupNumber  where t_groupmanage.hotelManage='代订旅游' and t_reserveplan.startDate between '".$firstday."' and '".$lastday."' order by t_groupmanage.id  limit ".$sr.",".$numPerPage  );
...
t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订旅游'  ".$sql." order by t_groupmanage.id" );
			     ...
			     @$sr=($pageNum-1)*$numPerPage;
			     $resultnow=mysqli_query($con,"select * from t_groupmanage right join t_reserveplan on
t_groupmanage.teamNumber=t_reserveplan.groupNumber where t_groupmanage.hotelManage='代订旅游'  ".$sql." order by t_groupmanage.id limit ".$sr.",".$numPerPage );
			     $resultnowarray=mysqli_fetch_all($resultnow,MYSQLI_ASSOC);
			 }
